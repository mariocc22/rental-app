...
function updateBasePrice(price) {
  // update in dom
  const basePriceElem = document.querySelector(
    ".detail-base ul > li:first-child span:last-child"
  );
...
function updateTotalPrice() {
  ...
  const priceDescription = document.querySelector(
    ".detail-total-price ul span:first-child"
  );
  const priceCalculation = document.querySelector(
    ".detail-total-price ul span:last-child"
  );
...
function calculatePrice() {
  const totalProps = USER_CART.equipments
...
function registerBookButton() {
  ...
  bookBtn.addEventListener("click", () => {
    ...
    window.location.href =
      window.location.origin +
      "/booking-confirmation.html?bookingInfo=" +
      bookingInfoBase64;
  });
}
...
function registerBundleSelection() {
  ...
  bundleSelectBtn.addEventListener("click", (event) => {
    ...
    const props = bundleInfo.equipment
      .filter((prop) => prop != "")
      .map((prop) => {
    ...
    const allProps = document.querySelectorAll(
      ".property-equipments input[type='checkbox']"
    );
...
function displaySelectedMenuInfo(elemValue) {
  ...
  const allElementsToHide = document.querySelectorAll(
    ".property-menu-info-wrapper > div"
  );
